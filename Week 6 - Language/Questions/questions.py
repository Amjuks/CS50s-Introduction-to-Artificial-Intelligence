...
def tokenize(document):
    ...
    return filter_words(nltk.word_tokenize(document.lower()))
...
def compute_idfs(documents):
    ...
    for doc in documents:
        for word in documents[doc]:
            idfs[word] = idfs_for_word(word, documents)

    return idfs

def idfs_for_word(word, documents):
    ...
    return math.log(total_documents / num_doc_containing_word)
...
def top_files(query, files, idfs, n):
    ...
    for file in files.keys():
        ...
        for word in files[file]:
            ...
            tfidf[file][word] = word_count * idfs[word]
    
    file_scores = {file: 0 for file in files.keys()}
    for file in files.keys():
        for word in query:
            if word in tfidf[file]:
                file_scores[file] += tfidf[file][word]

    top_scores = sorted(file_scores.keys(), key=lambda file: file_scores[file], reverse=True)
    return top_scores[:n]
...
def top_sentences(query, sentences, idfs, n):
    ...
    def query_term_density(sentence):
        ...
        for word in sentences[sentence]:
            if word in query:
                sen_scores[sentence]['density'] += sentences[sentence].count(word) / length
            
    for sentence in sentences:
        for word in query:
            if word in sentences[sentence]:
                sen_scores[sentence]['measure'] += idfs[word]
            
        query_term_density(sentence)
    
    top = sorted(
        sentences.keys(),
        key = lambda sen: (sen_scores[sen]['measure'], sen_scores[sen]['density']),
        reverse=True
    )    
